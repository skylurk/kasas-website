...
export function DornierPerformance() {
  ...
  const t = {
    ...
    hlTitle:    dark ? "text-white" : "text-gray-900",
    hlBody:     dark ? "text-white/60" : "text-gray-500",
    imgBorder:  dark ? "border-white/10" : "border-black/10",
    caption:    dark ? "text-white/30" : "text-gray-400",
    statVal:    dark ? "text-white" : "text-gray-900",
    statLbl:    dark ? "text-white/40" : "text-gray-400",
    statDivide: dark ? "bg-white/10" : "bg-black/10",
    statBorder: dark ? "border-white/10" : "border-black/10",
    cabinSub:   dark ? "text-white/40" : "text-gray-400",
    cabinBody:  dark ? "text-white/60" : "text-gray-500",
    watermark:  dark ? "opacity-5" : "opacity-[0.03] invert",
  }
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        ...
        {/* First split — highlights left, takeoff image right */}
          ...
          >
            {highlights.map((h, i) => (
              ...
              >
                ...
                <h3 className={`${t.hlTitle} text-base font-semibold`}>{h.title}</h3>
                <p className={`${t.hlBody} text-sm leading-relaxed`}>{h.body}</p>
              </motion.div>
            ))}
          </motion.div>
        ...
        {/* Second split — interior image left, cabin text right */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-20 items-center">
          ...
          >
            {/* Sketch watermark */}
            <div className={`absolute -right-8 -top-8 w-64 h-64 ${t.watermark} pointer-events-none select-none`}>
              <Image
            ...
            <h3 className={`${t.heading} text-2xl md:text-3xl font-semibold tracking-tight leading-snug`}>
              Quick-Change Cabin.
              <br />
              <span className={t.cabinSub}>Any Mission, Any Load.</span>
            </h3>

            <p className={`${t.cabinBody} text-sm md:text-base leading-relaxed`}>
            ...
            {/* Inline stats */}
            <div className={`flex items-center gap-8 pt-2 border-t ${t.statBorder} mt-2`}>
              <div>
                <p className={`text-2xl font-semibold ${t.statVal}`}>19</p>
                <p className={`text-xs ${t.statLbl} uppercase tracking-widest mt-0.5`}>Max seats</p>
              </div>
              <div className={`w-px h-8 ${t.statDivide}`} />
              <div>
                <p className={`text-2xl font-semibold ${t.statVal}`}>
                  1,800<span className="text-red-500/70 text-sm font-mono ml-1">kg</span>
                </p>
                <p className={`text-xs ${t.statLbl} uppercase tracking-widest mt-0.5`}>Max payload</p>
              </div>
              <div className={`w-px h-8 ${t.statDivide}`} />
              <div>
                <p className={`text-2xl font-semibold ${t.statVal}`}>3</p>
                <p className={`text-xs ${t.statLbl} uppercase tracking-widest mt-0.5`}>Configurations</p>
              </div>
            </div>
          </motion.div>
