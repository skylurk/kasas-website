...
export function DornierHero() {
  ...
  const t = {
    ...
    statLbl:  dark ? "text-white/50" : "text-gray-400",
    statBg:   dark ? "bg-black/80" : "bg-black/[0.03]",
    gridBg:   dark ? "bg-white/10" : "bg-black/10",
    gridBdr:  dark ? "border-white/10" : "border-black/10",
    scroll:   dark ? "text-white/30" : "text-gray-400",
    imgClass: dark ? "" : "invert",
  }

  return (
    <section
      className={`relative min-h-screen w-full ${t.section} flex flex-col items-center justify-center overflow-hidden px-6`}
    ...
    >
      ...
      >
        <Image
          ...
          fill
          priority
          className={`object-contain drop-shadow-[0_0_40px_rgba(220,38,38,0.4)] ${t.imgClass}`}
      ...
      >
        <h1 className={`text-6xl md:text-8xl font-semibold tracking-tighter ${t.heading}`}>
          Dornier
          <span className="text-red-500"> 228</span>
        </h1>
        <p className={`${t.sub} text-base md:text-lg max-w-xl leading-relaxed`}>
      ...
      <motion.div
        ...
        className={`relative z-10 mt-16 grid grid-cols-2 md:grid-cols-4 gap-px ${t.gridBg} border ${t.gridBdr} rounded-xl overflow-hidden w-full max-w-3xl`}
      >
        {[
          ...
          <div key={stat.label} className={`${t.statBg} px-6 py-6 flex flex-col gap-1`}>
            <p className={`text-2xl md:text-3xl font-semibold ${t.statVal} tracking-tight`}>
              {stat.value}
            </p>
            <p className={`text-xs ${t.statLbl} uppercase tracking-widest`}>
              {stat.label}
            </p>
          </div>
        ))}
      ...
      >
        <span className={`text-xs ${t.scroll} tracking-widest uppercase`}>Scroll</span>
        <motion.div
          animate={{ y: [0, 6, 0] }}
